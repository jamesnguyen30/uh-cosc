...
static pthread_mutex_t read_input_lock;
...
static int input_order_static = 0;
...
struct child_arg{
	string ip_and_subnet[MAX_COUNT];
	grandchild_arg grandchild_args[MAX_COUNT];
	//int order;
};
...
void * print_message(void * arg_void_ptr){
	...
	pthread_mutex_lock(&read_input_lock);

	struct child_arg* argument = (child_arg*) arg_void_ptr;		

	int order = input_order_static;
	input_order_static++;

	pthread_mutex_unlock(&read_input_lock);

	string ip_and_subnet = argument->ip_and_subnet[order];	
	pthread_t g_tid[2]; 

	grandchild_arg grandchild_args = argument->grandchild_args[order];
...
int main(){
	...
	while(getline(cin, line) && COUNT < MAX_COUNT){
		argument.ip_and_subnet[COUNT] = line;		
		COUNT++;
	}

	//cout<<"INPUT : "<<endl;
	//for(int i = 0;i<COUNT;i++){
	//	cout<<argument.ip_and_subnet[i]<<endl;		
	//}
	...
	for(int i = 0;i<COUNT;i++){
		...
		if(pthread_create(&tid[i], NULL, print_message, (void*)&argument)){
